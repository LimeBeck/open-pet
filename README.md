...
## Перетаскивание и мониторы

Питомца можно таскать мышью в пределах экрана; положение сохраняется
и переживает перезапуск.

Перенести его на другой монитор перетаскиванием нельзя: под Wayland клиент
не знает настоящей позиции указателя, поэтому «куда потащили» ему неизвестно.
Замерено — `QCursor::pos()` у layer-shell поверхности возвращает координаты
чужого экрана. Для переноса есть пункт трея «Перенести на другой экран»;
подробности в [ADR-002](docs/adr/0002-overlay-input-region.md).
...
Модель выбирается из выпадающего списка — он заполняется после нажатия
«Проверить связь». Поле остаётся редактируемым: модель может быть новее
нашего списка. Учти, что провайдер перечисляет и те модели, которые уже
не обслуживает: `gemini-2.0-flash` есть в списке, но на генерацию отвечает
404. Замеренная задержка на `gemini-flash-latest` — 3.2–4.1 с, отсюда
таймаут по умолчанию в 6 секунд.
